...
<div class="flex-1 flex flex-col">
    ...
    <!-- Documents Header -->
    <div class="p-6  min-h-scren">
        <!-- Sub-Header -->
        <div class="bg-transparent py-0 px-4 flex justify-between items-center mb-4">
            <h1 class="text-2xl font-semibold">Documents</h1>
            <div class="flex items-center space-x-3">
                <a href="{{ route('dashboard.document_create') }}"
        ...
        <div class="bg-white shadow p-4 rounded-2xl mb-6 ">
            ...
            <div class="flex items-center space-x-4 relative  mb-4 w-[400px]" id="divSearch">
                ...
                placeholder="Cari dokumen berdasarkan nama dokumen atau tag"
                class="w-full rounded-lg px-10 py-2 text-sm border-2 focus:border-blue-400" />
            ...
            <table class="w-full border-collapse">
                <thead id="documentTableHead"></thead>
...
$(document).ready(function () {
    const route_download = "{{ route('dashboard.document_download', [':id', ':filename']) }}"
    const route_detail = "{{ route('dashboard.document_detail', [':id', ':filename']) }}"
    const route_edit   = "{{ route('dashboard.document_edit', [':id', ':filename']) }}";
    const route_share  = "{{ route('dashboard.document_share_page', ':id') }}";
    ...
    function fetchDocuments(page = 1, q = "") {
        $.ajax({
            url: "{{ route('dashboard.documents.search') }}", // controller filter by auth()->id()
            type: "GET",
            data: { page: page, q: q },
            success: function (res) {
                ...
                if (res.data.length > 0) {
                    $.each(res.data, function (i, doc) {
                        // nomor
                        let nomor = res.from + i;

                        let downloadUrl = route_download
                            .replace(':id', doc.id)
                            .replace(':filename', doc.file_attachment.split('/').pop());

                        let detailUrl = route_detail
                            .replace(':id', doc.id)
                            .replace(':filename', doc.file_attachment.split('/').pop());
                        ...
                        let shareUrl = route_share.replace(':id', doc.id);

                        let deleteUrl = route_delete.replace(':id', doc.id);


                        rows += `
                            <tr class="border-t hover:bg-gray-100">
                                <td class="px-5 py-3">${nomor}</td>
                                <td class="px-5 py-3">${doc.nomor_dokumen}</td>
                                <td class="px-5 py-3">${doc.user ? doc.user.nama : '-'}</td>
                                <td class="px-5 py-3">${doc.nama_dokumen}</td>
                                <td class="px-5 py-3">
                                    ${doc.badge}
                                </td>
                                
                                <td class="px-5 py-3">${doc.log_dokumen?.tanggal_dibuat ? new Date(doc.log_dokumen.tanggal_dibuat).toLocaleDateString() : '-'}</td>
                                <td class="px-5 py-3">
                                    <div class="flex space-x-2 justify-center">
                                        ...
                                        <!-- SHARE -->
                                        <a href="${shareUrl}">
                                            <div class="bg-green-200 border flex items-center rounded-full p-2  hover:bg-green-300 text-green-600">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-5">
                    ...
                    // DISABLE search kalau tidak ada dokumen
                    // $("#searchInput").prop("disabled", true).val(""); 
                    rows = `<tr><td colspan="8" class="py-20 text-xl text-gray-500 font-light italic">Silahkan Buat Dokumen Terlebih Dahulu</td></tr>`;
                    $("#pagination").hide();
                }
